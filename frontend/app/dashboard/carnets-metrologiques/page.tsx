...
import {
  Plus, X, BookOpen, Truck, Hash,
  CalendarRange, CalendarCheck, Banknote, Percent, Wrench, Paperclip, MessageSquare,
} from 'lucide-react';
...
const COLUMNS = [
  ...
  { key: 'montantHT', label: 'Montant HT', render: (v: number) => v ? `${v} DH` : '—' },
  { key: 'montant',   label: 'Montant TTC', render: (v: number) => v ? `${v} DH` : '—' },
  {
    key: 'statut',
    label: 'Statut',
    render: (v: string) => (
...
export default function CarnetsMetrologiquesPage() {
  ...
  const { data, isLoading }    = useResource<any>('administratif', { page, search, type: 'carnet_metrologique' });
  const { data: vehiclesData } = useResource<any>('vehicles', { limit: 200 });
  const create                 = useCreateResource('administratif');

  // Auto-calculate TTC = HT * (1 + TVA/100)
  ...
  const handleSubmit = async () => {
    await create.mutateAsync({ ...form, type: 'carnet_metrologique', montant: montantTTC });
    handleClose();
  };
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          <h2 className="text-2xl font-bold text-gray-900">Carnets Métrologiques</h2>
          <p className="text-sm text-gray-500 mt-1">Gestion des carnets métrologiques</p>
        </div>
      ...
      {/* Table */}
      <SearchFilter onSearch={setSearch} placeholder="Rechercher un carnet métrologique..." filters={[]} />
      <DataTable
        columns={COLUMNS} data={data?.data || []} loading={isLoading}
        total={data?.total || 0} page={page} pages={data?.pages || 1}
        onPageChange={setPage} emptyMessage="Aucun carnet métrologique trouvé"
      />
          ...
          <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-lg flex flex-col overflow-hidden">
            ...
            <div className="flex items-center justify-between px-6 py-5"
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h3 className="text-lg font-bold text-white">Nouveau Carnet Métrologique</h3>
            ...
            <div className="overflow-y-auto max-h-[75vh] p-6 space-y-5">
              ...
              {/* Carnet métrologique + Véhicule */}
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  <IconLabel icon={BookOpen} color="#2563eb">Carnet métrologique *</IconLabel>
                  <input type="text" name="reference" value={form.reference} onChange={handleChange}
                    placeholder="Libellé du carnet"
              ...
              <div className="border border-gray-100 rounded-xl p-4 space-y-4 bg-gray-50/50">
                ...
                <div>
                  ...
                  <div className="border border-blue-200 rounded-lg px-3 py-2 text-sm bg-blue-50 text-blue-800 font-semibold">
                ...
                {/* Garage */}
                <div>
                  <IconLabel icon={Wrench} color="#0891b2">Garage</IconLabel>
